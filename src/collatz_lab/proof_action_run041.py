"""RUN-041 top-level replay after guarded SCC ranking."""
...
RUN_ID = "RUN-041-top-level-after-guarded-scc-ranking"
...
def _write_repo_replay_bundle(manifest: dict[str, Any], artifacts: dict[str, Path]) -> None:
    ...
    for name, path in artifacts.items():
        ...
        dst = store / f"run041_{name}{suffix}"
...
def run_top_level_after_guarded_scc_ranking(
    config_path: str | Path | None = None,
    *,
    out: str | Path | None = None,
) -> dict[str, Any]:
    cfg = load_yaml(config_path) if config_path else {}
    run_cfg = cfg.get("top_level_after_guarded_scc_ranking_run041", {}) if isinstance(cfg.get("top_level_after_guarded_scc_ranking_run041"), dict) else {}
    out_dir = Path(out or run_cfg.get("out_dir") or f"reports/runs/{RUN_ID}")
    out_dir.mkdir(parents=True, exist_ok=True)

    run040_result_path = Path(run_cfg.get("run040_result") or "reports/runs/RUN-040-guarded-scc-ranking-repair/run_result.json")
    run040_cert_path = Path(run_cfg.get("scc_guarded_ranking_certificate") or "reports/runs/RUN-040-guarded-scc-ranking-repair/scc_guarded_ranking_certificate.json")
    run040_result = _load_json(run040_result_path) if run040_result_path.exists() else {}
    scc_cert = _load_json(run040_cert_path) if run040_cert_path.exists() else {}
    scc_replay = replay_scc_guarded_ranking_certificate(scc_cert)
    if run040_result.get("status") != "PASS" or not scc_replay.get("accepted"):
        result = {
            "schema": "collatz_lab.run041_top_level_after_guarded_scc_ranking",
            "version": 1,
            "run_id": RUN_ID,
            ...
            "status": "BLOCKED_BY_RUN040_GUARDED_SCC_RANKING",
            "top_level_certificates_generated": 0,
            "top_level_replay_pass": 0,
            "strict_verifier": "FAIL",
            "proof_confidence_percent": 0.0,
            "run040_status": run040_result.get("status"),
            "run040_replay": scc_replay,
    ...
    manifest_hashes["scc_guarded_ranking_certificate"] = _sha256(run040_cert_path)
    ...
    manifest = {
        ...
        "source_files": [
            ...
            _source_entry(REPO_ROOT / "src/collatz_lab/proof_action_run040.py"),
            _source_entry(REPO_ROOT / "src/collatz_lab/proof_action_run041.py"),
    ...
    _write_json(result, out_dir / "run_result.json")
    if bool(run_cfg.get("write_repo_replay_bundle", True)):
        shutil.copy2(out_dir / "run_result.json", REPO_ROOT / "certificate_store/run041_run_result.json")
    return result
...
def main(argv: list[str] | None = None) -> None:
    ...
    result = run_top_level_after_guarded_scc_ranking(args.config, out=args.out)
