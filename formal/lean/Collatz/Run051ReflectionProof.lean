...
for reflected S4 parent transitions.  RUN-055 records that the current reflected
system projection still has no entry states, so global soundness cannot yet be
instantiated.  The final descent theorem is not declared here.
-/
...
theorem run051_entry_soundness_obstructed :
    ¬ UniversalEntrySemantics run051Bundle.system :=
  GlobalSemantics.reflected_system_not_universal_entry run051Bundle
